/**
 * POST /api/tmdb/movie/[id]/rating — set/replace a rating (1-10) for a movie.
 * DELETE — remove the current user's rating.
 *
 * Requires login (server-side `MoFlixxUser` session cookie).
 * Mirrors TMDB: POST /movie/{id}/rating, DELETE /movie/{id}/rating.
 */
...
export async function POST(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const result = await tmdbFetch<{ success?: boolean; status_message?: string }>(
    `/movie/${id}/rating`,
    {
      method: "POST",
...
export async function DELETE(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const result = await tmdbFetch<{ status: boolean; status_message?: string }>(
    `/movie/${id}/rating`,
    {
      method: "DELETE",
      sessionId: session.session_id,
